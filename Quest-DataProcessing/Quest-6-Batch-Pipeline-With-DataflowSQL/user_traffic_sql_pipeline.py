...
from apache_beam.transforms.sql import SqlTransform #Import SqlTransform
...
def run():
    ...
    (logs | 'RawToDict' >> beam.Map(lambda row : row._asdict())
          | 'WriteRawToBQ' >> beam.io.WriteToBigQuery( #Transform Raw To BigQuery
           raw_table_name,
    ...
    (logs | 'PerUserAggregations' >> SqlTransform(query, dialect='zetasql') #Apply SQL Transform

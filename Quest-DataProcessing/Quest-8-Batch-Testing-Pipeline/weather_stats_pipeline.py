...
class WeatherFormat(typing.NamedTuple):
    loc_id: str
    lat: float
    lng: float
    date: str
...
beam.coders.registry.register_coder(WeatherFormat, beam.coders.RowCoder)
...
class ConvertCsvToWeatherRecord(beam.DoFn):
    ...
    def process(self, line):
        ...
        yield WeatherFormat(**row)

class ConvertTempUnit(beam.DoFn):

    def process(self, row):
        row_dict = row._asdict()
        for field in ('low_temp', 'high_temp'):
            row_dict[field] = row_dict[field] * 1.8 + 32.0
        yield WeatherFormat(**row_dict)
...
class WeatherInsights(beam.PTransform):
    ...
    def expand(self, pcoll):
        ...
        results = (
            pcoll | "ParseCSV" >> beam.ParDo(ConvertCsvToWeatherRecord())
                  | "ConvertToF" >> beam.ParDo(ConvertTempUnit())

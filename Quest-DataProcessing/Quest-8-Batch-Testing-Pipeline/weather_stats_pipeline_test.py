...
from weather_stats_pipeline import *
...
class ConvertToWeatherRecordTest(unittest.TestCase):
    ...
    def test_convert_csv(self):
        ...
        with TestPipeline() as p:
            ...
            EXPECTED_OUTPUT = [WeatherFormat('x', 0.0, 0.0, '2/2/2021', 1.0, 2.0, 0.1)]

            input_lines = p | beam.Create(LINES) #Add PCollection from LINES

            output = input_lines | beam.ParDo(ConvertCsvToWeatherRecord())

            assert_that(output, equal_to(EXPECTED_OUTPUT)) #Add assert_that Statement
...
class ConvertTempUnitsTest(unittest.TestCase):
    ...
    def test_convert_temp_units(self):
        ...
        with TestPipeline() as p:
            ...
            RECORDS = [WeatherFormat('x', 0.0, 0.0, '2/2/2021', 1.0, 2.0, 0.1),
                       WeatherFormat('y', 0.0, 0.0, '2/2/2021', -3.0, -1.0, 0.3)]

            EXPECTED_RECORDS = [WeatherFormat('x', 0.0, 0.0, '2/2/2021', 33.8, 35.6, 0.1),
                                WeatherFormat('y', 0.0, 0.0, '2/2/2021', 26.6, 30.2, 0.3)]

            input_records = p | beam.Create(RECORDS)

            output = input_records | beam.ParDo(ConvertTempUnit())
...
class ComputeStatsTest(unittest.TestCase):
    ...
    def test_compute_statistics(self):
        ...
        with TestPipeline() as p:
            ...
            INPUT_RECORDS = [WeatherFormat('x', 0.0, 0.0, '2/2/2021', 33.8, 35.6, 0.1),
                             WeatherFormat('x', 0.0, 0.0, '2/3/2021', 41.6, 65.3, 0.2),
                             WeatherFormat('x', 0.0, 0.0, '2/4/2021', 45.3, 52.6, 0.2),
                             WeatherFormat('y', 0.0, 0.0, '2/2/2021', 12.8, 23.6, 0.1),
                             WeatherFormat('y', 0.0, 0.0, '2/3/2021', 26.6, 30.2, 0.3)]

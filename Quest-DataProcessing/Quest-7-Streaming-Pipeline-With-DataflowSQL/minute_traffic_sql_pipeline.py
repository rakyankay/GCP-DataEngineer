...
class ParseAndGetEventTimestamp(beam.PTransform):
    def expand(self, pcoll):
        return (
            pcoll
            | 'ParseJson' >> beam.Map(parse_json) #Apply parse_json Function
            | 'GetEventTimestamp' >> beam.ParDo(GetEventTimestampFn()) #Apply GetEventTimestamp DoFn
            )
...
def run():
    ...
    (p | 'ReadFromPubSub' >> beam.io.ReadFromPubSub(input_topic) #Read From Pub/Sub Topic

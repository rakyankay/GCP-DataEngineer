...
class ConvertToCommonLogFn(beam.DoFn):
  def process(self, element):
    try:
        ...
        yield beam.pvalue.TaggedOutput('parsed_row', CommonLog(**row)) #Execute While No Error
    except:
        yield beam.pvalue.TaggedOutput('unparsed_row', element.decode('utf-8')) #Generate Error Message
...
def run():
    ...
    options.view_as(GoogleCloudOptions).job_name = '{0}{1}'.format('streaming-late-data-pipeline-',time.time_ns())
    ...
    (rows.unparsed_row
        | 'BatchOver10s' >> beam.WindowInto(beam.window.FixedWindows(120),
                                            trigger=AfterProcessingTime(120), #Set Trigger
                                            accumulation_mode=AccumulationMode.DISCARDING) #Set Accumulation
        | 'WriteUnparsedToGCS' >> fileio.WriteToFiles(output_path, #Error Data Send To GCS
                                                      shards=1,
    ...
    (rows.parsed_row
        | "WindowByMinute" >> beam.WindowInto(beam.window.FixedWindows(int(window_duration)),
                                              trigger=AfterWatermark(late=AfterCount(1)), #Set Trigger
                                              allowed_lateness=int(allowed_lateness), #Set Allowed Lateness
                                              accumulation_mode=AccumulationMode.ACCUMULATING) #Set Accumulation 
